...
int count_iter (int consensus_size, int* consensus_length, int reads_size, int* reads_length) {
    int i, j, k, l;
    int count = 0;
    for (i = 0; i < consensus_size; i++) {
        int consensus_base =  consensus_length[i];
        ...
        for (j = 0; j < reads_size; j++) {
            int reads_base = reads_length[j];
            int local_reads_length = reads_length[j+1]-reads_length[j];
            count += (local_consensus_length - local_reads_length) * local_reads_length;
        }
        //consensus_base += local_consensus_length;
    }
    return count;
...
void generate_count(std::vector<char, aligned_allocator<char>>* consensus, std::vector<int, aligned_allocator<int>>* con_size, std::vector<int, aligned_allocator<int>>* con_base, \
    std::vector<char, aligned_allocator<char>>* reads, std::vector<int, aligned_allocator<int>>* reads_size, std::vector<int, aligned_allocator<int>>* reads_base, std::vector<char, aligned_allocator<char>>* qs, std::vector<int, aligned_allocator<int>>* new_ref_idx, int* test_indices) {
    for (size_t i = 0; i < reads_size->size() - 1; i++) {
	    ...
	    fprintf(stderr, "TEST %d ", test_indices[i]);
        ...
        int count = count_iter(con_size_local, &(con_base->data())[con_size_base], rs_size_local, &(reads_base->data()[reads_size_base]));
