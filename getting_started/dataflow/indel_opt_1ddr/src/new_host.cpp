...
void get_filename (char* file_prefix, char* con, char* reads, int number){
    char test_num[5];
...
//void parse(const char* file_prefix, int col_num, char* con_arr, int** con_len_arr, int* con_size, int*total_len) {
   ...
   //     //printf("con_arr_buffer_0[%d + %d]: %hhX \n", idx, offset, con_arr_buffer_0[idx]);
                        ...
                        input_arr -> pushback(m[p[k]]);
                    else{ // this is for weights 
                        input_arr -> pushback(p[k]);
                    }
                    //printf("%d\n", line_num* len + k);
        ...
        base += input_len;
        if(option < 3) len_base->pushback(prev_len_base + base);
        line_num++;
    }
    if(option < 3)size_base->pushback(prev_size_base + line_num); 
    //printf("%d\n", len);
...
//////OPENCL HOST CODE SETUP END////////
    ...
    for (int test_idx = 0; test_idx< num_tests; test_idx+=BATCH_SIZE ) {
        ...
        con_size_buffer->pushback(0);
        reads_size_buffer->pushback(0);
        ...
        con_base_buffer->pushback(0);
        reads_base_buffer->pushback(0);
        ...
        OCL_CHECK(qs[kernel_idx].enqueueMigrateMemObjects(inBufVec_arr.back(), 0/* 0 means from host*/, NULL, &events[0]));
        //q.finish();
        ...
        OCL_CHECK(qs[kernel_idx].enqueueTask(krnl_indels[kernel_idx], &events[0], &events[1]));
        //q.finish();

        //event.wait();
        //Copy Result from Device Global Memory to Host Local Memory
        //q.enqueueMigrateMemObjects(outBufVec, CL_MIGRATE_MEM_OBJECT_HOST);
        OCL_CHECK(qs[kernel_idx].enqueueMigrateMemObjects(outBufVec_arr.back(), CL_MIGRATE_MEM_OBJECT_HOST, &event[1], &events[2]));
        q.finish();
        ...
        compare_results(new_ref_idx_buffer, new_ref_idx_ref_buffer);


        //INST_0============================================//
       if (test_idx_0 < num_tests){ 
          reads_size_arr[test_idx_0] = reads_size_0;
          new_ref_idx_arr[test_idx_0] = new_ref_idx_buffer;
          eew_ref_idx_ref_arr[test_idx_0] = new_ref_idx_ref_0;
        }
